...
router.post('/create/:folderName', roleMiddleware('ADMIN', 'OWNER', 'USER'), async (req: any, res) => {
    try {
        const userEmail = req.user.email;
...
router.delete('/delete/:folderId', roleMiddleware('ADMIN', 'OWNER', 'USER'), async (req, res) => {
    try {
