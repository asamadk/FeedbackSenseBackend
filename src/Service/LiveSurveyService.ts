...
export const getLiveSurveyNodes = async(surveyId : string) : Promise<responseRest> => {
    ...
    try {
        ...
        console.log("🚀 ~ file: LiveSurveyService.ts:54 ~ getLiveSurveyNodes ~ error:", error);
        return getCustomResponse({},500,'Something went wrong.',false);   
    }
}
...
export const saveSurveyResponse = async (surveyId : string, responseData : any) => {
    try {
        ...
        console.log("🚀 ~ file: LiveSurveyService.ts:87 ~ saveSurveyResponse ~ error:", error)
    }
}
